...
import { generatePassword, checkPasswordStrength, getPasswordStrengthInfo, isValidEmail } from '@/lib/utils';
...
export function HRCreateForm() {
  ...
  const validate = () => {
    ...
    if (formData.email.trim() && !isValidEmail(formData.email.trim())) newErrors.email = 'Please enter a valid email address.';
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-lg space-y-5">
      ...
      <Input
        ...
        error={errors.email}
        hint="Used for sending automated approval/rejection emails to candidates"
        disabled={isLoading}
      />
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-4 p-3 bg-gray-50 rounded-2xl border border-gray-200">
          ...
          <div className="flex-1 min-w-0">
            ...
            />
            <p className="text-[11px] text-gray-400 mt-1">Upload a JPG, PNG, or WebP photo (Max 5MB)</p>
          </div>
        </div>
      </div>
